...
"""
    ...
    print(
        f"     LLM 선택 지속 시간: {nudge.duration_seconds}초 ({nudge.duration_seconds // 60}분 {nudge.duration_seconds % 60}초)"
    )
...
def send_intervention(state: InterventionState) -> dict:
    ...
    if state.get("intervention_needed", False):
        ...
        print(
            f"     평가 예정: {eval_scheduled_time} ({delay_minutes}분 후, 개입 유형: {intervention_type})"
        )
    else:
