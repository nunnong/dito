...
class ClosetRepository @Inject constructor(
    ...
    suspend fun getClosetItems(type: String, page: Int): Result<ClosetResponse> = withContext(Dispatchers.IO) {
        try {
            ...
            val response = apiService.getClosetItems(
            ...
            if (response.isSuccessful && response.body() != null) {
                if (response.body()!!.error == false) {
                    ...
                    Result.failure(Exception(response.body()!!.message ?: "옷장 아이템 로드 실패"))
                }
            } else {
